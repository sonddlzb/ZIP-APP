...
#pragma mark - Zipping
+ (BOOL)createZipFileAtPath:(NSString *)path withPaths:(NSArray<NSString *> *)paths withPassword:(nullable NSString *)password keepParentDirectory:(BOOL)keepParentDirectory 
{    
    ...
    if (success) {
        ...
        for (__strong NSString *tmpPath in paths) {
            BOOL isDir;
            [fileManager fileExistsAtPath:tmpPath isDirectory:&isDir];
            if (!isDir) {
                success &= [zipArchive writeFile:tmpPath withPassword:password];
            } else {
                NSDirectoryEnumerator *dirEnumerator = [fileManager enumeratorAtPath:tmpPath];
                ...
                for (__strong NSString *fileName in allObjects) {
                    NSString *fullFilePath = [tmpPath stringByAppendingPathComponent:fileName];
                    
                    if (keepParentDirectory) {
                        fileName = [tmpPath.lastPathComponent stringByAppendingPathComponent:fileName];
                    }
...
+ (NSArray<UZKFileInfo*> *)listFileInfo:(NSString *)path error:(NSError * __autoreleasing*)error
{
    // Guard against empty strings
    if (path.length == 0)
    {
    ...
    if (zip == NULL) {
        ...
        if (err != UNZ_OK) {
            NSDictionary *userInfo = @{NSLocalizedDescriptionKey: @"Error getting global info"};
            NSError *err = [NSError errorWithDomain:SSZipArchiveErrorDomain code:SSZipArchiveErrorCodeFailedOpenFileInZip userInfo:userInfo];
            if (error)
            {
                *error = err;
        ...
        {
            NSDictionary *userInfo = @{NSLocalizedDescriptionKey: @"Error going to first file in archive"};
            NSError *err = [NSError errorWithDomain:SSZipArchiveErrorDomain code:SSZipArchiveErrorCodeFailedOpenFileInZip userInfo:userInfo];
            if (error)
            {
                *error = err;
        ...
        for (NSUInteger i = 0; i < fileCount; i++) {
            UZKFileInfo *info = [SSZipArchive currentFileInZipInfo:zip error:error];
            
            if (info) {
                BOOL bExist = NO;
                for (UZKFileInfo *item in zipInfos) {
                    if ([item.filename isEqualToString:info.filename]) {
                        NSInteger index = [zipInfos indexOfObject:item];
                        item.numberFile += 1;
                        [zipInfos replaceObjectAtIndex:index withObject:item];
                        bExist = YES;
                        break;
                    }
                }
            ...
            if (err != UNZ_OK) {
                NSDictionary *userInfo = @{NSLocalizedDescriptionKey: @"Error navigating to next file"};
                NSError *err = [NSError errorWithDomain:SSZipArchiveErrorDomain code:SSZipArchiveErrorCodeFailedOpenFileInZip userInfo:userInfo];
                if (error)
                {
                    *error = err;
...
+ (UZKFileInfo *)currentFileInZipInfo:(zipFile)zip error:(NSError * __autoreleasing*)error {
    char filename_inzip[4096];
    unz_file_info64 file_info;
    
    int err = unzGetCurrentFileInfo64(zip, &file_info, filename_inzip, sizeof(filename_inzip), NULL, 0, NULL, 0);
    if (err != UNZ_OK) {
        *error = [NSError errorWithDomain:@"SSZipArchiveErrorDomain" code:SSZipArchiveErrorCodeFileInfoNotLoadable userInfo:@{NSLocalizedDescriptionKey: @"failed to retrieve info for file"}];
        return nil;
    }
    
    NSString *filename = [SSZipArchive figureOutCString:filename_inzip];
